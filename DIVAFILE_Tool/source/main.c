...
int main(int argc, char* argv[]) {
	...
	if (strcmp(argv[1], "e") == 0 || strcmp(argv[1], "E") == 0) { // extract
		...
		if (input.data == NULL) {
			fprintf_s(stderr, "fail to open source file: %s\n", argv[2]);
			exit(1);
		}
		if (decrypt_divafile(&input, &output) == 0) {
			fprintf_s(stderr, "extract fail.\n");
			exit(1);
		}
		if (write_to_file(argv[3], output.data, output.size) == 0) {
			fprintf_s(stderr, "fail to write file: %s\n", argv[3]);
			exit(1);
		}
		free(input.data);
		free(output.data);
		fprintf_s(stdout, "extract complete, %s -> %s", argv[2], argv[3]);
